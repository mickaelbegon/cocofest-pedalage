...
def run_comparison(
    ...
    for summary in summaries:
        print(
            ...
            f"{summary['solver_time_s']:.3f} | {summary['wall_time_s']:.3f} | {summary['final_wheel_angle']:.6f}"
        )
        print(
...
def main():
    parser = argparse.ArgumentParser(description="Compare IPOPT and ACADOS on the same compact cycling MHE.")
    parser.add_argument(
        "--model-path",
        type=str,
        default="../../msk_models/Wu/Modified_Wu_Shoulder_Model_Cycling.bioMod",
        help="Path to the torque-driven cycling model.",
    )
    parser.add_argument("--windows", type=int, default=2, help="Number of MHE windows to advance.")
    parser.add_argument("--window-len", type=int, default=5, help="Number of shooting intervals inside the MHE window.")
    parser.add_argument("--total-angle", type=float, default=-1.0, help="Final target wheel angle over the comparison.")
    parser.add_argument(
        "--acados-dir",
        type=str,
    ...
    parser.add_argument(
        "--acados-codegen-dir",
        type=str,
        default="result/acados/c_generated_code_compare",
        help="Directory used for ACADOS code generation.",
    )
    parser.add_argument("--ipopt-max-iter", type=int, default=500, help="Maximum IPOPT iterations.")
    parser.add_argument("--acados-max-iter", type=int, default=100, help="Maximum ACADOS SQP iterations.")
    ...
    run_comparison(
        ...
        acados_codegen_dir=str((REPO_ROOT / args.acados_codegen_dir).resolve()),
